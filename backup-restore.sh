...
BACKUP_EXCLUDE_PATTERNS="*.log *.tmp .git .env"
        ...
        case $choice in
            1)
                ...
                case "$bot_choice" in
                    ...
                    3) BOT_BACKUP_SELECTED="Remnashop"; bot_folder="remnashop" ;;
                    0) continue ;;
...
create_bot_backup() {
    ...
    # ------------------- Redis / Valkey dump -------------------
    local BOT_REDIS_CONTAINER="remnashop-redis"
    local BOT_REDIS_VOLUME="remnashop-redis-data"
    local BOT_REDIS_DUMP_FILE="bot_redis_${TIMESTAMP}.rdb"

    if [[ "$BOT_BACKUP_SELECTED" == "Remnashop" ]]; then
        if docker inspect "$BOT_REDIS_CONTAINER" > /dev/null 2>&1 && \
           docker container inspect -f '{{.State.Running}}' "$BOT_REDIS_CONTAINER" 2>/dev/null | grep -q "true"; then
            
            print_message "INFO" "Создание дампа Redis (Valkey)..."
            
            # Сохраняем текущий RDB-файл
            docker exec "$BOT_REDIS_CONTAINER" valkey-cli -a "${REDIS_PASSWORD:-}" SAVE > /dev/null 2>&1
            
            # Копируем dump.rdb из тома
            local temp_redis_dir="$BACKUP_DIR/redis_temp_$$"
            mkdir -p "$temp_redis_dir"
            
            if docker cp "$BOT_REDIS_CONTAINER:/data/dump.rdb" "$temp_redis_dir/$BOT_REDIS_DUMP_FILE" 2>/dev/null; then
                mv "$temp_redis_dir/$BOT_REDIS_DUMP_FILE" "$BACKUP_DIR/$BOT_REDIS_DUMP_FILE"
                rm -rf "$temp_redis_dir"
                print_message "SUCCESS" "Дамп Redis успешно сохранён."
                BACKUP_ITEMS+=("$BOT_REDIS_DUMP_FILE")
            else
                print_message "WARN" "Не удалось скопировать dump.rdb из контейнера Redis."
                rm -rf "$temp_redis_dir"
            fi
        else
            print_message "WARN" "Контейнер Redis ($BOT_REDIS_CONTAINER) не запущен → пропускаем бэкап Redis."
        fi
    fi
...
restore_bot_backup() {
    ...
    while true; do
        ...
        case "$bot_choice" in
            ...
            3) selected_bot_name="Remnashop"; break ;;
    ...
    elif [[ "$selected_bot_name" == "Remnashop" ]]; then
    ...
    # ------------------- Восстановление Redis dump -------------------
    if [[ "$selected_bot_name" == "Remnashop" ]]; then
        local BOT_REDIS_DUMP=$(find "$temp_restore_dir" -name "bot_redis_*.rdb" | head -n 1)
        
        if [[ -n "$BOT_REDIS_DUMP" ]]; then
            print_message "INFO" "Обнаружен дамп Redis. Восстанавливаем..."
            
            # Останавливаем Redis, если запущен
            docker compose stop remnashop-redis 2>/dev/null || true
            
            # Удаляем старый том, если нужно
            if docker volume ls -q | grep -Fxq "remnashop-redis-data"; then
                docker volume rm "remnashop-redis-data" 2>/dev/null || true
            fi
            
            # Запускаем только Redis
            docker compose up -d remnashop-redis
            
            print_message "INFO" "Ожидание готовности Redis..."
            sleep 5  # обычно хватает
            
            # Копируем новый dump.rdb
            if docker cp "$BOT_REDIS_DUMP" "remnashop-redis:/data/dump.rdb"; then
                print_message "SUCCESS" "Дамп Redis скопирован в контейнер."
                
                # Заставляем Redis его подгрузить (перезапуск)
                docker compose restart remnashop-redis
                sleep 3
            else
                print_message "ERROR" "Не удалось скопировать дамп Redis в контейнер."
            fi
        else
            print_message "INFO" "Дамп Redis в бэкапе не найден → пропускаем."
        fi
    fi
