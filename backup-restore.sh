...
create_bot_backup() {
    ...
    # ------------------- Redis / Valkey dump (Remnashop / Бот от Snoups) -------------------
    if [[ "$BOT_BACKUP_SELECTED" == "Бот от Snoups" ]]; then
...
restore_bot_backup() {
    ...
    # ------------------- Восстановление Redis dump (Remnashop / Бот от Snoups) -------------------
    if [[ "$selected_bot_name" == "Бот от Snoups" ]]; then
